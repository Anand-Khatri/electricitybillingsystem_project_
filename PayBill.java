...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter){
        ...
        try{
            ...
            rs = st.executeQuery( "select * from customer where meter = '"+meter+"'");
            while(rs.next()){
                l11.setText(rs.getString("meter"));
                l12.setText(rs.getString("name"));
            }
                        rs = st.executeQuery( "select * from bill where meter = '"+meter+"' AND month = 'January' ");
            while(rs.next()){
        ...
        c1.addItemListener(new ItemListener(){
            ...
            public void itemStateChanged(ItemEvent ae){
                try{
                   conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ebill", "root", "Khatri@1234");
                st = conn.createStatement();
                   rs = st.executeQuery("select * from bill where meter = '"+meter+"' AND month = '"+c1.getSelectedItem()+"'");
                    while(rs.next()){
                        l13.setText(rs.getString("units"));
                        l14.setText(rs.getString("total_bill"));
                        l15.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            try{
                conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ebill", "root", "Khatri@1234");
                st = conn.createStatement();
              rs = st.executeQuery("update bill status = 'Paid' where meter = '"+meter+"' AND month = '"+c1.getSelectedItem()+"'");  
            }catch(Exception e){}
