...
public class UpdateInformation extends JFrame implements ActionListener{
    ...
    UpdateInformation(String meter,String name){
        this.meter1 = meter;
        this.nameU=name;
        ...
        try{
            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ebill", "root", "Khatri@1234");
                ...
                rs = st.executeQuery("select * from customer where meter = '"+meter1+"'");
            while(rs.next()){
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
               conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ebill", "root", "Khatri@1234");
                st = conn.createStatement();
                rs = st.executeQuery("update customer set address = '"+s3+"', city = '"+s4+"', state = '"+s5+"', email = '"+s6+"', phone = '"+s7+"' where meter = '"+meter1+"'");
